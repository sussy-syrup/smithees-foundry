...
public class HoeToolTypeModel implements IToolTypeModel {
    ...
    public void render(HashMap<String, FabricBakedModel> PART_MODELS, ItemStack stack, RenderContext context)
    {
        ...
        if(!(nbt == null)) {
            ...
            PART_MODELS.get(ForgeMaterialRegistry.getMaterial(nbt.getString(ToolItem.HEAD_KEY)).getName() + "_hoe_head").emitItemQuads(null, null, context);
            if(nbt.contains(ToolItem.BINDING_KEY))
            PART_MODELS.get(ForgeMaterialRegistry.getMaterial(nbt.getString(ToolItem.BINDING_KEY)).getName() + "_hoe_binding").emitItemQuads(null, null, context);
            if(nbt.contains(ToolItem.HANDLE_KEY))
            PART_MODELS.get(ForgeMaterialRegistry.getMaterial(nbt.getString(ToolItem.HANDLE_KEY)).getName() +"_hoe_handle").emitItemQuads(null, null, context);
        }
        else
        {

...
public class CastingBlockCategory implements DisplayCategory<CastingBlockDisplay> {

    private static final Identifier TEXTURE = new Identifier(SMCompatMain.MODID, "textures/gui/casting_display.png");

    @Override
    public Renderer getIcon() {
        return EntryStacks.of(BlocksRegistry.CASTING_BASIN_BLOCK);
    }
    ...
    @Override
    public CategoryIdentifier<? extends CastingBlockDisplay> getCategoryIdentifier() {
    ...
    @Override
    public List<Widget> setupDisplay(CastingBlockDisplay display, Rectangle bounds) {
        ...
        widgets.add(Widgets.createArrow(new Point(bounds.x + 38, bounds.y + 17)).animationDurationTicks(160));

        widgets.add(Widgets.createSlot(new Point(bounds.x + 121, bounds.y + 17)).entries(display.getOutputEntries().get(0)));

        widgets.add(Widgets.createArrow(new Point(bounds.x + 93, bounds.y + 17)).animationDurationTicks(540));

        return widgets;
    }

    @Override
    public DisplayRenderer getDisplayRenderer(CastingBlockDisplay display) {
